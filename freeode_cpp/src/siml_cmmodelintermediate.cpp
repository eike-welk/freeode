...
/*!
Transform a process from presentation of the parser into an intermediate model,
that can be used for code generation. This funcion does error and consistency
checking, so that the code generation can be done without much error checking.

As the target of code generation is an othe high level language the parer's
representation and the intermediate representation are quite similar though.
The intermediate representation does not contain submodels. It contains all
needed variables and parameters, as well as all algorithms (SET, INITIAL,
EQUATION sections).
The number of parameters is reduced due to the parameter propagation rules.
Futurous versions of the class should perform optimization.

The entities from the input (compositeProcess) are copied into *this and are
then modified, to give the intermediate model.

@param compositeProcess Input which was generated by the parser.
@return true if the conversion finished successfully, false otherwise.
*/
bool
siml::CmModelIntermediate::createIntermediateModel( CmModelDescriptor const & compositeProcess)
{
    //collect parameters, variables and equations from all models and put them into one big model
    createFlatModel( compositeProcess);
...
void siml::CmModelIntermediate::checkErrors()
{
    ...
    {
        if( itMem->isStateVariable ) { noAssignmentYet.insert( itMem->name); }
    }
